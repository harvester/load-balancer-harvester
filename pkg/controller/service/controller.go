...
func (h Handler) OnChange(key string, service *coreV1.Service) (*coreV1.Service, error) {
	...
	if v, ok := labels[entry.KeyLabelEntry]; !ok || v != "true" {
		return nil, nil
	}
	klog.Infof("service configuration %s has been changed, spec: %+v", service.Name, service.Spec)
	...
	lbCopy.Status.ExternalAddress = service.Spec.LoadBalancerIP
	lbCopy.Status.InternalAddress = service.Spec.ClusterIP

	if _, err := h.lbClient.Update(lbCopy); err != nil {
		return nil, fmt.Errorf("update loadbalcer %s failed, error: %w", lbCopy.Name, err)
	}
