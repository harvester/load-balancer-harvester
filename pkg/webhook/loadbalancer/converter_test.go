...
func convert(converter conversion.Converter, obj, expectedObj runtime.Object, toVersion string) error {
	...
	if !reflect.DeepEqual(convertedObj, expectedObj) {
		return fmt.Errorf("expected: %+v, got: %+v", expectedObj, convertedObj)
	}
